...
const PI_TOOL_NAMES: &[&str] = &["edit", "write", "replace", "rename"];
...
impl AgentCheckpointPreset for PiPreset {
    fn run(&self, flags: AgentCheckpointFlags) -> Result<AgentRunResult, GitAiError> {
        ...
        let model_from_hook = model.trim().to_string();
...
impl PiPreset {
    fn validate_tool_name(tool_name: &str) -> Result<(), GitAiError> {
        if PI_TOOL_NAMES.contains(&tool_name) {
            Ok(())
        } else {
...
mod tests {
    ...
    fn test_pi_payload_parsing_before_edit() {
        ...
        assert_eq!(result.agent_id.model, "anthropic/claude-sonnet-4-5");
    ...
    #[test]
    fn test_pi_rejects_unsupported_canonical_tool_name() {
        let hook_input = json!({
        ...
        let error = PiPreset
            ...
            .expect_err("unsupported canonical tool_name should fail");

        assert!(error.to_string().contains("Unsupported Pi tool_name: bash"));
    }
